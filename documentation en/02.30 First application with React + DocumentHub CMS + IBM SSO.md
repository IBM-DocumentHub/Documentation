...
function App() {
  ...
  useEffect(() => {
    const getData = async () => {
      const data = await documenthub.getDocumentFileAsHtml('IcCVI6RO', 'documentation', 'en', '01 About DocumentHub.md')
...
function App() {
  ...
  useEffect(() => {
    const getData = async () => {
      const data = await documenthub.getDocument('IcCVI6RO', 'documentation', 'en')
      setFetchedData(data);
    };
    getData();
  }, []);
