...
- getTokenCookie() - get the caasToken cookie. If the token missing returns null. Is the token is expired then it's deleted and returns null.
...
- async ensureW3idLogin(libraryId, redirectUrl) - if user is not logged in, it does the w3id SSO login and then redirects to redirectUrl or back to the current page.
